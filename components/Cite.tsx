...
function CiteWeb(props: CiteWebProps): React.ReactElement {
    ...
    if (props.date)
        date = <>{" ("}<DateTime value={props.date} />)</>;
    ...
    const accessDate = <>Retrieved <DateTime value={props.accessDate} />{". "}</>;

    let archived: JSX.Element | null = null;
    if (props.archiveUrl) {
        if (props.archiveDate)
            archived = <>{" "}<A href={props.archiveUrl}>Archived</A> from the original on <DateTime value={props.archiveDate} />.</>;
        else
